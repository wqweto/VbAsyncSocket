...
#define snew(type) ( (type *) getContext()->m_CoTaskMemAlloc(sizeof(type)) )
#define snewn(n, type) ( (type *) getContext()->m_CoTaskMemAlloc((n) * sizeof(type)) )
#define sresize(x, n, type) ( (type *) getContext()->m_CoTaskMemRealloc((x), (n) * sizeof(type)) )
#define sfree(x) ( getContext()->m_CoTaskMemFree((x)) )
inline void smemclr(void *b, size_t n) {
    if (b && n > 0)
...
static BignumInt g_bnZero[1] = { 0 };
static BignumInt g_bnOne[2] = { 1, 1 };

#define Zero ((Bignum)bnZero)
#define One ((Bignum)bnOne)
...
inline static Bignum newbn(int length)
{
    Bignum b;
...
inline static void bn_restore_invariant(Bignum b)
{
    while (b[0] > 1 && b[b[0]] == 0)
	    b[0]--;
}

inline static Bignum copybn(Bignum orig)
{
...
inline static void freebn(Bignum b)
{
    /*
 ...
 */
static Bignum modpow_simple(Bignum base_in, Bignum exp, Bignum mod)
{
...
static Bignum modsub(const Bignum a, const Bignum b, const Bignum n)
{
    Bignum a1, b1, ret;
...
static Bignum bignum_from_bytes_le(const unsigned char *data, int nbytes)
{
    Bignum result;
    int w, i;
...
//static Bignum bignum_from_decimal(const char *decimal)
//{
...
inline uint8_t random_byte() { return 0; }

static Bignum bignum_random_in_range(const Bignum lower, const Bignum upper)
{
    Bignum ret = NULL;
    unsigned char *bytes;
 ...
 */
static Bignum bignum_lshift(Bignum a, int shift)
{
    Bignum ret;
 ...
 */
static Bignum bigmul(Bignum a, Bignum b)
{
 ...
 * Simple addition.
 */
static Bignum bigadd(Bignum a, Bignum b)
{
    int alen = a[0], blen = b[0];
 ...
 * positive numbers).
 */
static Bignum bigsub(Bignum a, Bignum b)
{
    int alen = a[0], blen = b[0];
 ...
 */
static Bignum bignum_from_long(unsigned long n)
{
    const int maxwords =
 ...
 */
static Bignum bignum_add_long(Bignum number, unsigned long n)
{
    const int maxwords =
        (sizeof(unsigned long) + sizeof(BignumInt) - 1) / sizeof(BignumInt);
    Bignum ret;
